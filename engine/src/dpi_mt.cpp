...
private:
    ...
    void writeJsonReport(const std::string &json_file)
    {
        if (json_file.empty())
        {
            return;
        ...
        if (!json.is_open())
        {
            std::cerr << "[JSON] Cannot open output file: "
                      << json_file << "\n";
            return;
        }
        ...
        // Summary
        json << "  \"summary\": {\n";
        json << "    \"totalPackets\": " << stats_.total_packets.load() << ",\n";
        json << "    \"totalBytes\": " << stats_.total_bytes.load() << ",\n";
        json << "    \"tcpPackets\": " << stats_.tcp_packets.load() << ",\n";
        json << "    \"udpPackets\": " << stats_.udp_packets.load() << ",\n";
        json << "    \"forwardedPackets\": " << stats_.forwarded.load() << ",\n";
        json << "    \"droppedPackets\": " << stats_.dropped.load() << "\n";
        json << "  },\n";
...

        {
            ...
            std::vector<std::pair<AppType, uint64_t>> apps(
...

        {
            ...
            for (const auto &[sni, app] : stats_.detected_snis)
            {
                ...
                if (++index < stats_.detected_snis.size())
                {
                    json << ",";
                }
        ...
        {
            total_flows += fp->getFlows().size();
        }

        for (const auto &fp : fps_)
        {
            for (const auto &[tuple, flow] : fp->getFlows())
            {
                json << "    {\n";

                json << "      \"sourceIp\": \""
                     << ipToString(tuple.src_ip)
                     << "\",\n";

                json << "      \"destinationIp\": \""
                     << ipToString(tuple.dst_ip)
                     << "\",\n";
                ...
                json << "      \"protocol\": "
                     << static_cast<int>(tuple.protocol) << ",\n";

                json << "      \"application\": \""
                     << escapeJson(appTypeToString(flow.app_type))
                     << "\",\n";
        ...
        std::cout << "[JSON] Analysis report written to: "
                  << json_file << "\n";
    }
...
Options:
  ...
  --json <file>          Write machine-readable analysis results
