  ...
  const chips = useMemo((): AppChip[] => {
    ...
    return Array.from(protoMap.entries())
      ...
      .map(([name, packets]) => ({
        ...
        packets,
        blocked: false,
        color: name === 'TCP' ? '#3b82f6' : name === 'UDP' ? '#22c55e' : '#f59e0b',
      }));
  }, [data, viewMode]);
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3 flex-shrink-0">
        <div>
          ...
          <p className={cn('text-xs mt-0.5', isLight ? 'text-gray-500' : 'text-muted')}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={cn(
              ...
              isLight ? 'text-gray-400 hover:bg-gray-100' : 'text-muted hover:bg-white/5',
            )}
          >
            <Maximize2 size={14} />
          </button>
      ...
      <div className="relative flex-1" style={{ minHeight: 300 }}>
        ...
        >
          ...
          {hasData && chips.slice(0, chipCount).map((chip, i) => {
          ...
          <g transform={`translate(${LEFT_X - 60}, ${HUB_Y - 38})`}>
            ...
            <Monitor
              ...
              color="#38bdf8"
            />
            <text x={70} y={40} textAnchor="middle" fill={isLight ? '#1e293b' : 'white'} fontSize={10} fontWeight="600">
          ...
          {/* ── Hub Circle ── */}
          {glowEnabled && (
            <circle cx={HUB_X} cy={HUB_Y} r="48" fill="#38bdf8" fillOpacity="0.04">
          ...
          <circle
            ...
            strokeOpacity="0.7"
          />
          <circle cx={HUB_X} cy={HUB_Y} r="34" fill="none" stroke="#38bdf8" strokeWidth="0.5" strokeOpacity="0.3" />
          {/* Pulse icon inside hub */}
          <text x={HUB_X} y={HUB_Y - 6} textAnchor="middle" fill="#38bdf8" fontSize="18">∿</text>
          <text x={HUB_X} y={HUB_Y + 10} textAnchor="middle" fill="#94a3b8" fontSize="7.5" fontWeight="500">NetInspect</text>
          ...
          {chips.slice(0, chipCount).map((chip, i) => {
            ...
            return (
              ...
              >
                ...
                <rect width="3" height={chipH} rx="2" fill={chip.color} opacity="0.8" />
                {/* App name */}
                <text x={18} y={chipH / 2 + 4} fill={isLight ? '#1e293b' : 'white'} fontSize={9.5} fontWeight="600">
                  {chip.name.length > 14 ? chip.name.slice(0, 14) + '…' : chip.name}
                </text>
                {/* Packet count */}
          ...
          {!hasData && (
            <g>
              <text x={W / 2} y={H / 2 - 10} textAnchor="middle" fill="#94a3b8" fontSize="12">
                Upload a PCAP file to visualize traffic
              </text>
              <text x={W / 2} y={H / 2 + 8} textAnchor="middle" fill="#64748b" fontSize="10">
      ...
      <div className="flex items-center gap-5 px-4 pb-3 flex-shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className={cn('text-xs', isLight ? 'text-gray-500' : 'text-muted')}>Allowed Traffic</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-danger" />
          <span className={cn('text-xs', isLight ? 'text-gray-500' : 'text-muted')}>Blocked Traffic</span>
        </div>
      </div>
    </div>
  );
